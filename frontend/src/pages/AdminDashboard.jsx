...
function AdminsTab() {
  const { data: list, loading, error, refetch } = useDataFetch('http://localhost:8080/api/admins')
    ...
    <div className="card shadow-sm w-100">
      <div className="card-body">
        ...
        <Modal show={deleteState.open} onClose={() => setDeleteState({ id: null, reason: '', open: false })} title="Delete Admin">
          ...
          <div className="modal-footer">
            <button className="btn btn-secondary" onClick={() => setDeleteState({ id: null, reason: '', open: false })}>Cancel</button>
            <button className="btn btn-danger" onClick={performDelete} disabled={!deleteState.reason.trim()}>Delete</button>
          </div>
        </Modal>

        <Modal show={editState.open} onClose={() => setEditState({ open: false, id: null, firstName: '', lastName: '', email: '', permissions: '' })} title="Edit Admin">
          <div className="modal-body">
            <div className="row g-2">
              <div className="col-md-6"><input className="form-control" placeholder="First name" value={editState.firstName} onChange={(e) => setEditState({ ...editState, firstName: e.target.value })} /></div>
              <div className="col-md-6"><input className="form-control" placeholder="Last name" value={editState.lastName} onChange={(e) => setEditState({ ...editState, lastName: e.target.value })} /></div>
              <div className="col-12"><input className="form-control" placeholder="Email" type="email" value={editState.email} onChange={(e) => setEditState({ ...editState, email: e.target.value })} /></div>
              <div className="col-12"><input className="form-control" placeholder="Permissions" value={editState.permissions} onChange={(e) => setEditState({ ...editState, permissions: e.target.value })} /></div>
            </div>
          </div>
          <div className="modal-footer">
...
function StudentsTab() {
  ...
  const fetchStudentDetails = async (studentId) => {
    ...
    try {
      ...
      try {
        const enrollmentRes = await axios.get(`http://localhost:8080/api/enrollments/student/${studentId}`)
        enrollments = enrollmentRes.data?.data || []
        courses = await Promise.all(enrollments.map(e => 
          axios.get(`http://localhost:8080/api/courses/${e.courseId}`).then(r => r.data?.data || null).catch(() => null)
        ))
      } catch (e) { console.error('Failed to load enrollments:', e) }

      let certificates = []
      try {
        const certRes = await axios.get(`http://localhost:8080/api/certificates/student/${studentId}`)
        certificates = certRes.data?.data || certRes.data || []
      } catch (e) { console.log('Certificates API not available yet') }

      let badges = []
      try {
        const badgeRes = await axios.get(`http://localhost:8080/api/badges/student/${studentId}`)
        badges = badgeRes.data?.data || badgeRes.data || []
      } catch (e) { console.log('Badges API not available yet') }

      setDetails({ certificates, badges, enrollments, courses, loading: false })
                ...
                <div className="list-group">
                  {details.certificates.map((cert, idx) => (
                    <div key={idx} className="list-group-item">
                      <div>
                        <strong>{cert.courseTitle || `Certificate ${cert.certificateId || idx + 1}`}</strong>
                        {cert.uniqueCode && <div className="text-muted small">Code: {cert.uniqueCode}</div>}
                        {cert.issueDate && <div className="text-muted small">Issued: {new Date(cert.issueDate).toLocaleDateString()}</div>}
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {details.badges.map((badge, idx) => (
                    <div key={idx} className="list-group-item">
                      <div className="d-flex align-items-center">
                        ...
                        <div>
                          <strong>{badge.name || `Badge ${badge.badgeId || idx + 1}`}</strong>
                          {badge.description && <div className="text-muted small">{badge.description}</div>}
                          {badge.earnedAt && <div className="text-muted small">Earned: {new Date(badge.earnedAt).toLocaleDateString()}</div>}
                        </div>
                      </div>
                    </div>
                  ))}
            ...
            {details.enrollments.length > 0 ? (
              <div className="list-group mt-2">
                {details.enrollments.map((enrollment, idx) => {
                  ...
                  return (
                    <ListGroupItem key={enrollment.enrollmentId || idx}
                      title={course?.title || `Course ID: ${enrollment.courseId}`}
                      description={course?.description}
                      badge={enrollment.completionStatus || 'In Progress'}
                      badgeVariant={enrollment.completionStatus === 'Completed' ? 'success' : 'warning'}
                      extra={enrollment.enrollmentDate && <span className="text-muted small ms-2">Enrolled: {new Date(enrollment.enrollmentDate).toLocaleDateString()}</span>} />
                  )
                })}
              </div>
...
function InstructorsTab() {
  ...
  const fetchInstructorDetails = async (instructorId) => {
    ...
    try {
      const instructorRes = await axios.get(`http://localhost:8080/api/instructors/${instructorId}`)
      const instructor = instructorRes.data
      let courses = []
      try {
        const coursesRes = await axios.get(`http://localhost:8080/api/courses/instructor/${instructorId}`)
            ...
            {details.courses.length > 0 ? (
              <div className="list-group mt-2">
                {details.courses.map((course) => (
                  <ListGroupItem key={course.courseId} title={course.title} description={course.description} 
                    badge={course.status || 'Draft'} badgeVariant={course.status === 'Published' ? 'success' : 'secondary'} />
                ))}
              </div>
  ...
  const fetchCourseDetails = async (courseId) => {
    ...
    try {
      ...
      if (course.instructorId) {
        try {
          const instructorRes = await axios.get(`http://localhost:8080/api/instructors/${course.instructorId}`)
          instructor = instructorRes.data
        } catch (e) { console.error('Failed to load instructor:', e) }
      }
  ...
  const fetchAllEnrollments = async () => {
    ...
    try {
      ...
      const enrollmentPromises = students.map(async (student) => {
        try {
          const enrollmentRes = await axios.get(`http://localhost:8080/api/enrollments/student/${student.id}`)
          ...
          const enrollmentsWithDetails = await Promise.all(
            studentEnrollments.map(async (enrollment) => {
              try {
                const courseRes = await axios.get(`http://localhost:8080/api/courses/${enrollment.courseId}`)
                const course = courseRes.data?.data || courseRes.data
                return {
                  ...enrollment,
                  studentName: `${student.firstName} ${student.lastName}`,
                  courseName: course?.title || `Course ID: ${enrollment.courseId}`
                }
              } catch (e) {
                return {
                  ...enrollment,
    ...
    <div className="card shadow-sm w-100">
      <div className="card-body">
        ...
        {loading ? (
          ...
          <DataTable columns={columns} data={enrollments} emptyMessage="No enrollments found" />
        )}
      </div>
    </div>
  )
